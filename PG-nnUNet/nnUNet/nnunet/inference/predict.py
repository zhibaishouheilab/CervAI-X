...
from nnunet.inference.segmentation_export import save_segmentation_nifti_from_softmax, save_segmentation_nifti, save_heatmap_nifti
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    for preprocessed in preprocessing:
        ...
        all_output_files.append(output_filename)  # 修正这里，应该是output_filename而不是all_output_files
        ...
        #print("d",d.shape) # d (1, 1, 963, 918)
        # trainer.predict_preprocessed_data_return_seg_and_softmax 返回三个值，第一个是segmentation map，第二个是softmax概率，第三个是关键点检测热图
        # 一次性获取所有输出，避免重复计算
        outputs = trainer.predict_preprocessed_data_return_seg_and_softmax(
        ...
        seg_map = outputs[0]
        softmax = outputs[1]
        det_heatmap = outputs[2]  # 获取关键点检测热图
        print("det_heatmap", det_heatmap.shape)  # 检查热图的形状
        #det_heatmap = det_heatmap[0]
        
        # 热图文件名：在原始输出文件名基础上添加后缀
        heatmap_filename = output_filename.replace('.nii.gz', '_heatmap.nii.gz')
        
        # 保存热图到临时文件（如果需要）
        bytes_per_voxel_heatmap = 4
        if all_in_gpu:
            bytes_per_voxel_heatmap = 2
        
        # 检查热图是否过大需要临时存储
        if np.prod(det_heatmap.shape) > (2e9 / bytes_per_voxel_heatmap * 0.85):
            print("Heatmap output is large. Saving temporarily to disk")
            np.save(heatmap_filename[:-7] + ".npy", det_heatmap)
            det_heatmap_ref = heatmap_filename[:-7] + ".npy"
        else:
            det_heatmap_ref = det_heatmap
        
        # 为热图启动保存任务
        results.append(pool.starmap_async(save_heatmap_nifti,
                                          ((det_heatmap_ref, heatmap_filename, dct, 1,  # 使用线性插值
