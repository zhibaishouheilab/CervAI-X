...
def save_segmentation_nifti_from_softmax(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
    ...
    print(f"seg_old_size_postprocessed shape: {seg_old_size_postprocessed.shape}")
...
def save_heatmap_nifti(heatmap: Union[str, np.ndarray], 
                       out_fname: str,
                       properties_dict: dict, 
                       order: int = 1,
                       heatmap_postprocess_fn: callable = None,
                       heatmap_postprocess_args: tuple = None,
                       resampled_npz_fname: str = None,
                       ...
                       verbose: bool = True):
    """
    保存多通道热图（回归输出）到NIFTI文件
    """
    if verbose: 
        print(f"Saving heatmap to {out_fname}")

    # 加载数据（如果是文件路径）
    if isinstance(heatmap, str):
        assert isfile(heatmap), f"File {heatmap} does not exist"
        if heatmap.endswith('.npy'):
            heatmap = np.load(heatmap)
        elif heatmap.endswith('.npz'):
            heatmap = np.load(heatmap)['heatmap']
        # 不删除文件，因为可能是临时文件

    # 确保热图有正确的维度 (C, D, H, W) 或 (C, H, W)
    assert len(heatmap.shape) in [3, 4], "Heatmap must be 3D or 4D array"
    ...
    # 重采样热图到原始尺寸
    if np.any(np.array(heatmap.shape[1:]) != np.array(shape_original_after_cropping)):
        if force_separate_z is None:
            if get_do_separate_z(properties_dict.get('original_spacing')):
                do_separate_z = True
                lowres_axis = get_lowres_axis(properties_dict.get('original_spacing'))
        ...
        if verbose: 
            print("Resampling heatmap...")
            
        print("heatmap shape before resampling:", heatmap.shape)
        
        # 对热图进行重采样
        resampled_heatmap = resample_data_or_seg(
            heatmap, 
            shape_original_after_cropping, 
            is_seg=False,  # 作为连续数据处理
            axis=lowres_axis, 
            order=order, 
        ...
        resampled_heatmap = heatmap

    # 保存重采样后的热图（可选）
    if resampled_npz_fname is not None:
        np.savez_compressed(resampled_npz_fname, heatmap=resampled_heatmap.astype(np.float16))
        save_pickle(properties_dict, resampled_npz_fname[:-4] + ".pkl")
    ...
    if bbox is not None:
        full_size_heatmap = np.zeros([resampled_heatmap.shape[0]] + list(shape_original_before_cropping))
        
        for c in range(resampled_heatmap.shape[0]):
            channel_data = resampled_heatmap[c]
            channel_full = np.zeros(shape_original_before_cropping)
            
            # 应用边界框
            for dim in range(len(shape_original_before_cropping)):
                bbox[dim][1] = min(bbox[dim][0] + channel_data.shape[dim], 
                                  shape_original_before_cropping[dim])
            
            slices = tuple(slice(bbox[dim][0], bbox[dim][1]) for dim in range(len(shape_original_before_cropping)))
            channel_full[slices] = channel_data
            full_size_heatmap[c] = channel_full
    else:
        full_size_heatmap = resampled_heatmap
    
    print(f"Full size heatmap shape: {full_size_heatmap.shape}")

    # 应用后处理（如果有）
    if heatmap_postprocess_fn is not None:
        processed_heatmap = heatmap_postprocess_fn(np.copy(full_size_heatmap), *heatmap_postprocess_args)
    else:
        processed_heatmap = full_size_heatmap
    
    print(f"Processed heatmap shape: {processed_heatmap.shape}")

    # 保存为NIFTI（SimpleITK要求通道维度在最后）
    # 将通道维度移动到最后一个位置 (C, D, H, W) -> (D, H, W, C)
    if len(processed_heatmap.shape) == 3:  # 2D图像 (C, H, W)
        heatmap_for_sitk = np.moveaxis(processed_heatmap, 0, -1)  # (H, W, C)
    else:  # 3D图像 (C, D, H, W)
        heatmap_for_sitk = np.moveaxis(processed_heatmap, 0, -1)  # (D, H, W, C)
        
    print(f"Heatmap shape for saving: {heatmap_for_sitk.shape}")
    #heatmap_for_sitk = heatmap_for_sitk.squeeze()  # 去除单通道维度
    for i in range(heatmap_for_sitk.shape[3]):

        heatmap_for_sitk_i = heatmap_for_sitk[:,:,:,i]
    
        sitk_image = sitk.GetImageFromArray(heatmap_for_sitk_i)
        sitk_image.SetSpacing(properties_dict['itk_spacing'])
        sitk_image.SetOrigin(properties_dict['itk_origin'])
        sitk_image.SetDirection(properties_dict['itk_direction'])
        out_fname_i = out_fname.replace('.nii.gz', f'_{i}.nii.gz')
        sitk.WriteImage(sitk_image, out_fname_i)

    # 保存未后处理的版本（可选）
    if non_postprocessed_fname is not None:
        if len(full_size_heatmap.shape) == 3:
            raw_heatmap = np.moveaxis(full_size_heatmap, 0, -1)
        else:
            raw_heatmap = np.moveaxis(full_size_heatmap, 0, -1)
        
        raw_sitk_image = sitk.GetImageFromArray(raw_heatmap)
        raw_sitk_image.SetSpacing(properties_dict['itk_spacing'])
        raw_sitk_image.SetOrigin(properties_dict['itk_origin'])
        raw_sitk_image.SetDirection(properties_dict['itk_direction'])
        sitk.WriteImage(raw_sitk_image, non_postprocessed_fname)