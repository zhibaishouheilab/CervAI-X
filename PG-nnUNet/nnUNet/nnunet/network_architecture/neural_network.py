...
class SegmentationNetwork(NeuralNetwork):
    def __init__(self):
        ...
        self.keypoint_channels = 56  # 新增关键点检测热图的通道数
            ...
            with torch.no_grad():
                if self.conv_op == nn.Conv3d:
                    ...
                    if use_sliding_window:
                        ...
                        print(len(res), "results returned from _internal_predict_3D_2Dconv_tiled")
                    else:
    ...
    def _internal_maybe_mirror_and_pred_2D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
        ...
        x = maybe_to_torch(x)
        #print("num_classes", self.num_classes) # num_classes 15
        result_torch = torch.zeros([x.shape[0], self.num_classes] + list(x.shape[2:]), dtype=torch.float)
        det_result_torch = torch.zeros([x.shape[0], self.keypoint_channels] + list(x.shape[2:]), dtype=torch.float)
        #print(result_torch.shape) # torch.Size([1, 15, 1024, 1280])

        if torch.cuda.is_available():
            x = to_cuda(x, gpu_id=self.get_device())
            result_torch = result_torch.cuda(self.get_device(), non_blocking=True)
            det_result_torch = det_result_torch.cuda(self.get_device(), non_blocking=True)
        ...
        for m in range(mirror_idx):
            if m == 0:
                #print(x.shape) # torch.Size([1, 1, 1024, 1280])
                #print(len(self(x))) # 2
                pred = self.inference_apply_nonlin(self(x)[0])
                #pred_1 = self.inference_apply_nonlin(self(x)[1])
                #print(pred_1.shape) # torch.Size([1, 56, 1024, 1280])
                result_torch += 1 / num_results * pred
                det_result_torch += 1 / num_results * self(x)[1]
                

            if m == 1 and (1 in mirror_axes):
                pred = self.inference_apply_nonlin(self(torch.flip(x, (3, )))[0])
                #print("pred shape", pred.shape) # torch.Size([1, 15, 1024, 1280])
                result_torch += 1 / num_results * torch.flip(pred, (3, ))
                det_result_torch += 1 / num_results * torch.flip(self(torch.flip(x, (3, )))[1], (3, ))

            if m == 2 and (0 in mirror_axes):
                pred = self.inference_apply_nonlin(self(torch.flip(x, (2, )))[0])
                result_torch += 1 / num_results * torch.flip(pred, (2, ))
                det_result_torch += 1 / num_results * torch.flip(self(torch.flip(x, (2, )))[1], (2, ))

            if m == 3 and (0 in mirror_axes) and (1 in mirror_axes):
                pred = self.inference_apply_nonlin(self(torch.flip(x, (3, 2)))[0])
                result_torch += 1 / num_results * torch.flip(pred, (3, 2))
                det_result_torch += 1 / num_results * torch.flip(self(torch.flip(x, (3, 2)))[1], (3, 2))

        if mult is not None:
            result_torch[:, :] *= mult

        return result_torch, det_result_torch
    ...
    def _internal_predict_2D_2Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        if all_in_gpu:
            ...
            aggregated_results = torch.zeros([self.num_classes] + list(data.shape[1:]), dtype=torch.half,
                                             device=self.get_device())
            aggregated_det_results = torch.zeros([self.keypoint_channels] + list(data.shape[1:]), dtype=torch.half,
                                                 device=self.get_device())
            ...
            aggregated_nb_of_predictions = torch.zeros([self.num_classes] + list(data.shape[1:]), dtype=torch.half,
                                                       device=self.get_device())
            aggregated_det_nb_of_predictions = torch.zeros([self.keypoint_channels] + list(data.shape[1:]), dtype=torch.half,
                                                          device=self.get_device())
        else:
            if use_gaussian and num_tiles > 1:
                ...
                add_for_nb_of_preds = np.ones(patch_size, dtype=np.float32)
            aggregated_results = np.zeros([self.num_classes] + list(data.shape[1:]), dtype=np.float32)
            aggregated_det_results = np.zeros([self.keypoint_channels] + list(data.shape[1:]), dtype=np.float32)
            aggregated_nb_of_predictions = np.zeros([self.num_classes] + list(data.shape[1:]), dtype=np.float32)
            aggregated_det_nb_of_predictions = np.zeros([self.keypoint_channels] + list(data.shape[1:]), dtype=np.float32)
        ...
        for x in steps[0]:
            ...
            for y in steps[1]:
                ...
                predicted_patch = self._internal_maybe_mirror_and_pred_2D(
                    ...
                    gaussian_importance_map)[0][0]
                predicted_det_patch = self._internal_maybe_mirror_and_pred_2D(
                    data[None, :, lb_x:ub_x, lb_y:ub_y], mirror_axes, do_mirroring,
                    gaussian_importance_map)[1][0]

                if all_in_gpu:
                    predicted_patch = predicted_patch.half()
                    predicted_det_patch = predicted_det_patch.half()
                else:
                    predicted_patch = predicted_patch.cpu().numpy()
                    predicted_det_patch = predicted_det_patch.cpu().numpy()

                aggregated_results[:, lb_x:ub_x, lb_y:ub_y] += predicted_patch
                aggregated_det_results[:, lb_x:ub_x, lb_y:ub_y] += predicted_det_patch
                aggregated_nb_of_predictions[:, lb_x:ub_x, lb_y:ub_y] += add_for_nb_of_preds
                aggregated_det_nb_of_predictions[:, lb_x:ub_x, lb_y:ub_y] += add_for_nb_of_preds
        ...
        slicer_0 = tuple(
            [slice(0, aggregated_results.shape[i]) for i in
             range(len(aggregated_results.shape) - (len(slicer) - 1))] + slicer[1:])
        slicer_1 = tuple(
            [slice(0, aggregated_det_results.shape[i]) for i in
             range(len(aggregated_det_results.shape) - (len(slicer) - 1))] + slicer[1:])
        aggregated_results = aggregated_results[slicer_0]
        aggregated_nb_of_predictions = aggregated_nb_of_predictions[slicer_0]
        aggregated_det_results = aggregated_det_results[slicer_1]
        ...
        if all_in_gpu:
            ...
            aggregated_det_results = aggregated_det_results.detach().cpu().numpy()

        if verbose: print("prediction done")
        # print(class_probabilities.shape) # (15, 963, 918)
        # print(predicted_segmentation.shape) # (963, 918)
        # print(aggregated_det_results.shape) # (56, 963, 918)
        return predicted_segmentation, class_probabilities, aggregated_det_results
    ...
    def _internal_predict_3D_2Dconv_tiled(self, x: np.ndarray, patch_size: Tuple[int, int], do_mirroring: bool,
        ...
        for s in range(x.shape[1]):
            pred_seg, softmax_pres, pred_det = self._internal_predict_2D_2Dconv_tiled(
                x[:, s], step_size, do_mirroring, mirror_axes, patch_size, regions_class_order, use_gaussian,
                pad_border_mode, pad_kwargs, all_in_gpu, verbose)

            predicted_segmentation.append(pred_seg[None])
            softmax_pred.append(softmax_pres[None])
            pred_des.append(pred_det[None])

        predicted_segmentation = np.vstack(predicted_segmentation)
        softmax_pred = np.vstack(softmax_pred).transpose((1, 0, 2, 3))
        pred_des = np.vstack(pred_des).transpose((1, 0, 2, 3))
        # print(predicted_segmentation.shape)  # (1, 963, 918)
        print("softmax_pred shape",softmax_pred.shape)  # (15, 1, 963, 918)
        # print(pred_des.shape)  # (56, 1, 963, 918)

        return predicted_segmentation, softmax_pred, pred_des
