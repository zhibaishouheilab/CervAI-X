...
class DataLoader2D(SlimDataLoaderBase):
    ...
    def determine_shapes(self):
        ...
        if isfile(self._data[k]['data_file'][:-4] + ".npy"):
            ...
            case_all_data = np.load(self._data[k]['data_file'])['data']
        #print("case_all_data shape", case_all_data.shape)
        num_color_channels = case_all_data.shape[0] - num_seg -num_heatmap
        data_shape = (self.batch_size, num_color_channels, *self.patch_size)
        seg_shape = (self.batch_size, num_seg, *self.patch_size)
        heatmap_shape = (self.batch_size, num_heatmap, *self.patch_size)
    ...
    def generate_train_batch(self):
        ...
        #print(self.data_shape,self.seg_shape,self.heatmap_shape)
        #print(selected_keys)

        data = np.zeros(self.data_shape, dtype=np.float32)
        seg = np.zeros(self.seg_shape, dtype=np.float32)
        heatmap = np.zeros(self.heatmap_shape, dtype=np.float32)
        #print(self.data_shape,self.seg_shape,self.heatmap_shape)
        ...
        for j, i in enumerate(selected_keys):
            ...
            #print(case_all_data[0].shape, case_all_data[1].shape,case_all_data[-1:].shape)

            case_all_data_donly = np.pad(case_all_data[0:1], ((0, 0),
                                                              (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                              (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0))),
                                         self.pad_mode, **self.pad_kwargs_data)
            
            #print(case_all_data[0].shape,case_all_data[1:-1].shape, case_all_data[-1:].shape)
            case_all_data_heatmaponly = np.pad(case_all_data[1:-1], ((0, 0),
                                                              (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                              (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0))),
                                         self.pad_mode, **self.pad_kwargs_data)
            ...
            heatmap[j] = case_all_data_heatmaponly
            
        keys = selected_keys
        return {'data': data, 'seg': seg, 'heatmap':heatmap, 'properties': case_properties, "keys": keys}
